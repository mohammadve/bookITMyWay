...
class RegionDialogFragmentMulti : DialogFragment() {
    var regionSelectionAdapterMulti: RegionSelectionAdapterMulti? = null
    ...
    private var TAG: String? = RegionDialogFragmentMulti::class.java.name
    var apiService: ApiInterface? = null


    companion object {
        fun newInstance(from: String): RegionDialogFragmentMulti {
            val args = Bundle()
            args.putString("name", from)
//            args.putInt("value", value)
            val fragment = RegionDialogFragmentMulti()
...
//        dialog.window!!.setLayout(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
    ...
    private fun initializeUIComponent(regionResponse: ArrayList<RegionModel>) {
...
//        progress_select_region = mView!!.findViewById(R.id.progress_select_region) as ProgressBar
        ...
        regionSelectionAdapterMulti = RegionSelectionAdapterMulti(activity!!, regionResponse) { serviceModel ->
    ...
    private fun handleResults(regionResponse: RegionResponse) {
        isCancelable = true
...
//            UiValidator.displayMsgSnack(main_cord, context as DashBoardActivity, cityResponse.message)
    ...
    fun fetchRegion() {
        if (AppUtils.isInternetConnected(activity)) {
            ...
            apiService!!.getRegion(SharedPreferenceManager.getRegisterCountryDetails().data?.countryCode).subscribeOn(Schedulers.io())
    ...
    interface MultiRegionSelectionInterface {
        fun done(bean: ArrayList<RegionModel>, fromWhere: String?)
    }
