...
class BusinessCategoryAdapter(val mContext: Context, val cityModellist: ArrayList<AllCategoryModel>, val clickListener: (AllCategoryModel) -> Unit) : RecyclerView.Adapter<BusinessCategoryAdapter.ResultItemViewHolder>(), Filterable {
    private var filterdata: FilterText = FilterText()

    var lisData: ArrayList<AllCategoryModel> = cityModellist
    val filterlist: ArrayList<AllCategoryModel> = cityModellist
    ...
    class ResultItemViewHolder(itemView: View, val clickListener: (AllCategoryModel) -> Unit) : RecyclerView.ViewHolder(itemView) {

        fun bind(mContext: Context, offModel: AllCategoryModel) {
    ...
    private inner class FilterText : Filter() {
        ...
        override fun performFiltering(constraint: CharSequence): Filter.FilterResults {
            ...
            if (charString.isEmpty()) {
                ...
                val filteredList = ArrayList<AllCategoryModel>()
                ...
                if (filteredList.size == 0) {
                    var noDataBean = AllCategoryModel()
                    noDataBean.name = "NODATA"
                    noDataBean.category_id = ""
        ...
        override fun publishResults(constraint: CharSequence, results: Filter.FilterResults) {
            lisData = results.values as ArrayList<AllCategoryModel>
