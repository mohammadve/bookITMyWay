...
class BookRideTourBusActivity : BaseActivity(), View.OnClickListener, RegionDialogFragmentSingle.SingleRegionSelectionInterface , CountryDialogFragment.countrySelectionInterface {
    ...
    var TAG: String = BookRideTourBusActivity::class.java.simpleName
    ...
    var serviceDetailModel: VendorServiceDetailModel = VendorServiceDetailModel()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_book_ride_tourbus)
    ...
    fun init() {
        ...
        setDisplayFragment(1, resources.getString(R.string.book_ride), false)
    }
    ...
    fun setDisplayFragment(number: Int, title: String, removeStack: Boolean) {
        ...
        when (number) {
            ...
            3 -> replaceFragment(BookingInfromationFragment.newInstance(), removeStack)
            4 -> replaceFragment(BookingFragmentFinal.newInstance(), removeStack)
    ...
    fun handleBackPress() {
        when {
            ...
            fragmentManager!!.findFragmentById(R.id.flContentnew) is BookingFragmentSeviceDetail -> {
                if (!isFromSearch) {
                setTitleBar(resources.getString(R.string.book_ride))
                fragmentManager!!.popBackStackImmediate() } else {
                    finish()
                }
            }

            fragmentManager!!.findFragmentById(R.id.flContentnew) is BookingInfromationFragment -> {
                setTitleBar(resources.getString(R.string.business_profile))
                fragmentManager!!.popBackStackImmediate()
            }
            fragmentManager!!.findFragmentById(R.id.flContentnew) is BookingFragmentFinal -> {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1113) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (fragment != null && fragment.isVisible) {
                    if (fragment is BookingFragmentFinal) fragment.hitAPIForConfirm(result);
                }
            }
        }
    }
