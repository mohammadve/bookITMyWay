...
class BookAppointmentDoctorActivity : BaseActivity(), View.OnClickListener,
    ...
    var TAG: String = BookAppointmentDoctorActivity::class.java.simpleName
    ...
    fun showTimeSelectionDialog(date: String) {
        ...
        customerTimeSlotRequest.subcategory_id = AppConstants.SUBCAT_HEALTH_DOCTOR.toString()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_book_appointment_doctor)
    ...
    fun init() {
        ...
        setDisplayFragment(1, resources.getString(R.string.doctordentist), false)
    }
    ...
    fun setDisplayDialog(number: Int, title: String) {
        when (number) {
            ...
            8 -> showSpecialitySelectionDialog(title)
            9 -> showCountrySelectionDialog()
            10 -> showSpecialitySelectionDialog(title)
        }
    }
    ...
    fun handleBackPress() {
        when {
            ...
            fragmentManager!!.findFragmentById(R.id.flContentnew) is BookingFragmentSeviceDetail -> {
                if (!isFromSearch) {
                    setTitleBar(resources.getString(R.string.doctordentist))
    ...
    override fun specialitySelectionInterface(bean: AppointmentReasonModel, fromWhere: String?) {
        if (specialityDialogFragment != null) {
            ...
            val fragment = manager!!.findFragmentById(R.id.flContentnew)
            if (fragment != null && fragment.isVisible) {
                if (fragment is AppointmentInformationFragment) fragment.updateSelectedSpeciality(bean)
            }
        }
    }
