...
import com.virtual.customervendor.customer.ui.adapter.ClothingSelectionAdapterMulti
...
class ClothingCatFragmentMulti : DialogFragment() {
    var regionSelectionAdapterMulti: ClothingSelectionAdapterMulti? = null
    ...
    private var serviceModelList = ArrayList<ClothingCategoryModel>()
    private var mregionSelectionInterface: ClothingSelectionInterface? = null
    private var fromWhere: String? = null
    private var TAG: String? = ClothingCatFragmentMulti::class.java.name
    var apiService: ApiInterface? = null


    companion object {
        fun newInstance(from: String): ClothingCatFragmentMulti {
            val args = Bundle()
            args.putString("name", from)
//            args.putInt("value", value)
            val fragment = ClothingCatFragmentMulti()
...
//        dialog.window!!.setLayout(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
    ...
    private fun initializeUIComponent(regionResponse: ArrayList<ClothingCategoryModel>) {
...
//        progress_select_region = mView!!.findViewById(R.id.progress_select_region) as ProgressBar
        ...
        tv_ok_btn!!.setOnClickListener { mregionSelectionInterface!!.done(serviceModelList) }
        regionSelectionAdapterMulti = ClothingSelectionAdapterMulti(activity!!, regionResponse) { serviceModel ->
    ...
    private fun handleResults(regionResponse: ClothingCategoryResponse) {
        isCancelable = true
...
//            UiValidator.displayMsgSnack(main_cord, context as DashBoardActivity, cityResponse.message)
    ...
    override fun onAttach(context: Context?) {
        ...
        try {
            mregionSelectionInterface = context as ClothingSelectionInterface
    ...
    fun fetchRegion() {
        if (AppUtils.isInternetConnected(activity)) {
            ...
            apiService!!.storeClothSubCategory
                    ?.subscribeOn(Schedulers.io())
                    ?.observeOn(AndroidSchedulers.mainThread())
                    ?.subscribe(object : Observer<ClothingCategoryResponse> {
                        override fun onSubscribe(d: Disposable) {
                            AppLog.e(TAG, "onSubscribe")
                        }

                        override fun onNext(regionResponse: ClothingCategoryResponse) {
    ...
    interface ClothingSelectionInterface {
        fun done(bean: ArrayList<ClothingCategoryModel>)
    }
