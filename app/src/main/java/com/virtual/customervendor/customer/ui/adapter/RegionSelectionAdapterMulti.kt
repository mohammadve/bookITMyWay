...
class RegionSelectionAdapterMulti(val mContext: Context, val offermodel: ArrayList<RegionModel>, val clickListener: (ArrayList<RegionModel>) -> Unit) : RecyclerView.Adapter<RegionSelectionAdapterMulti.ResultItemViewHolder>(), Filterable {
    ...
    inner class ResultItemViewHolder(itemView: View, val clickListener: (ArrayList<RegionModel>) -> Unit) : RecyclerView.ViewHolder(itemView) {

        fun bind(mContext: Context, serviceModelList: ArrayList<RegionModel>, position: Int) {
            if (!serviceModelList.get(position).regionname.equals("NODATA")) {
                itemView.tv_noCountrySearch.visibility = View.GONE
                itemView.txt_title.visibility = View.VISIBLE
                itemView.txt_title.text = serviceModelList.get(position).regionname
