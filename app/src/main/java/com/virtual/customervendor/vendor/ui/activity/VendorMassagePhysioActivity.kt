...
class VendorMassagePhysioActivity : BaseActivity(), View.OnClickListener, CityDialogFragment.citySelectionInterface, RegionDialogFragmentSingle.SingleRegionSelectionInterface, UploadBussinessImage.IBussinessImage1 {

    var datetime: String? = null
    var manager: FragmentManager? = null
    var TAG: String = VendorMassagePhysioActivity::class.java.simpleName
    ...
    override fun selectedCityUpdate(bean: CityModel, fromWhere: String?) {
        ...
        if (cityDialogFragment != null) {
            ...
            if (fragment != null && fragment.isVisible) {
                if (fragment is VendorAppOneFragment) {
                    (fragment as VendorAppOneFragment).updateSelectedCity(bean)
                }
            }
        }
    }
    ...
    override fun regionSelectionInterface(bean: RegionModel, fromWhere: String?) {
        ...
        if (regionDialogFragmentsingle != null) {
            ...
            if (fragment != null && fragment.isVisible) {
                if (fragment is VendorAppOneFragment) {
                    (fragment as VendorAppOneFragment).updateSelectedRegion(bean)
                }
            }
        }
    }
...
//        setToolBar()
    ...
    fun init() {
        ...
        if (isFromEdit) {
            massageServiceRequest = CachingManager.getVendorDoctorInfo()
            if (fragmentCount == 1) {
    ...
    fun getMassageFieldPojo(): VendorHealthServiceRequest {
        return massageServiceRequest
    }
    ...
    fun hitApi(taxi_Service_Request: VendorHealthServiceRequest) {
        if (AppUtils.isInternetConnected(this)) {
            ...
            taxi_Service_Request.business_subcategory_id = AppConstants.SUBCAT_HEALTH_PHYSIO.toString()
    ...
    private fun handleResults(userResponse: TaxiServiceResponse) {
        ...
        if (userResponse.status.equals(AppConstants.KEY_SUCCESS)) {
            CachingManager.setVendorDoctorInfo(massageServiceRequest)
            if (!isFromEdit) {
                if (imageFiles != null) {
    ...
    public fun uploadPic(files: ArrayList<File>, bussinessId: String) {
        ...
        if (files != null && files.size > 0) {
            files.subList(0, massageServiceRequest.business_images.size).clear()
        }
