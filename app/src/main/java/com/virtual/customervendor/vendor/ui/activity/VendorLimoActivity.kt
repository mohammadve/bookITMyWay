...
class VendorLimoActivity : BaseActivity(), View.OnClickListener, CityDialogFragment.citySelectionInterface, RegionDialogFragmentMulti.MultiRegionSelectionInterface, RegionDialogFragmentSingle.SingleRegionSelectionInterface, UploadBussinessImage.IBussinessImage1 {
    ...
    var TAG: String = VendorLimoActivity::class.java.simpleName
...
//        setToolBar()
    ...
    fun init() {
        ...
        if (isFromEdit) {
            limo_Service_Request = CachingManager.getVendorTaxiInfo()
            if (fragmentCount == 1) {
    ...
    fun getLimoFieldPojo(): Ven_Taxi_Service_Request {
        return limo_Service_Request
    }
    ...
    fun hitApi(limo_Service_Request: Ven_Taxi_Service_Request) {
        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
        limo_Service_Request.business_category_id = AppConstants.CAT_TRANSPORTATION.toString()
        limo_Service_Request.business_subcategory_id = AppConstants.SUBCAT_TRANS_LIMO.toString()
        apiInterface?.taxiServiceSetUp("Bearer " + SharedPreferenceManager.getAuthToken(), limo_Service_Request)
    ...
    fun hitApiEdit(limo_Service_Request: Ven_Taxi_Service_Request) {
        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
        limo_Service_Request.action = AppConstants.ACTION_EDIT
        apiInterface?.taxiServiceSetUp("Bearer " + SharedPreferenceManager.getAuthToken(), limo_Service_Request)
                ?.subscribeOn(Schedulers.io())
                ?.observeOn(AndroidSchedulers.mainThread())
                ?.subscribe(object : Observer<TaxiServiceResponse> {
    ...
    private fun handleResults(userResponse: TaxiServiceResponse) {
        ...
        CachingManager.setVendorTaxiInfo(limo_Service_Request)
        if (isFromEdit) {
...
//    public fun uploadPic(file: File, bussinessId: String) {
//        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
//        UploadBussinessImage.uploadUserPicOnServer(file, bussinessId, this)
//    }

    public fun uploadPic(files: ArrayList<File>, bussinessId: String) {
        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
        if (files != null && files.size > 0) {
            files.subList(0, limo_Service_Request.business_images.size).clear()
        }
