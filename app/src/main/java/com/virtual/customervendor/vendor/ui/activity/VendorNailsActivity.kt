...
class VendorNailsActivity : BaseActivity(), View.OnClickListener, CityDialogFragment.citySelectionInterface, RegionDialogFragmentSingle.SingleRegionSelectionInterface, UploadBussinessImage.IBussinessImage1 {

    var datetime: String? = null
    var manager: FragmentManager? = null
    var TAG: String = VendorNailsActivity::class.java.simpleName
    ...
    var nail_Service_Request = VendorHealthServiceRequest()
...
//        setToolBar()
    ...
    fun init() {
        ...
        if (isFromEdit) {
            nail_Service_Request = CachingManager.getVendorDoctorInfo()
            if (fragmentCount == 1) {
    ...
    fun getNailFieldPojo(): VendorHealthServiceRequest {
        return nail_Service_Request
    }
    ...
    fun hitApi(nail_Service_Request: VendorHealthServiceRequest) {
        if (AppUtils.isInternetConnected(this)) {
            ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
            if(isFromEdit)
                nail_Service_Request.action = AppConstants.ACTION_EDIT
            nail_Service_Request.business_subcategory_id = AppConstants.SUBCAT_HEALTH_NAIL.toString()
            nail_Service_Request.business_category_id = AppConstants.CAT_HEALTH_BODYCARE.toString()
            apiInterface?.healthServiceSetUp("Bearer " + SharedPreferenceManager.getAuthToken(), nail_Service_Request)
    ...
    private fun handleResults(userResponse: TaxiServiceResponse) {
        ...
        if (userResponse.status.equals(AppConstants.KEY_SUCCESS)) {
            CachingManager.setVendorDoctorInfo(nail_Service_Request)
            if (!isFromEdit) {
                if (imageFiles != null) {
    ...
    public fun uploadPic(files: ArrayList<File>, bussinessId: String) {
        ...
        if (files != null && files.size > 0) {
            files.subList(0, nail_Service_Request.business_images.size).clear()
        }
