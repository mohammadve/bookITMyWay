...
class VendorTourBusActivity : BaseActivity(), View.OnClickListener, CityDialogFragment.citySelectionInterface, RegionDialogFragmentMulti.MultiRegionSelectionInterface, RegionDialogFragmentSingle.SingleRegionSelectionInterface, UploadBussinessImage.IBussinessImage1 {
    ...
    override fun done(bean: ArrayList<RegionModel>, fromWhere: String?) {
        ...
        if (regionDialogFragmentMulti != null) {
            ...
            if (fragment != null && fragment.isVisible) {
                if (fragment is VendorTaxiTwoFragment) {
                    (fragment as VendorTaxiTwoFragment).updateSelectedServiceArea(bean)
                }
            }
        }
    }
    ...
    override fun selectedCityUpdate(bean: CityModel, fromWhere: String?) {
        ...
        if (cityDialogFragment != null) {
            ...
            val fragment = manager!!.findFragmentById(R.id.flContentnew)
            if (fragment != null && fragment.isVisible) {
                if (fragment is VendorTaxiOneFragment) {
                    (fragment as VendorTaxiOneFragment).updateSelectedCity(bean)
                }
            }
        }
    }
    ...
    var TAG: String = VendorTourBusActivity::class.java.simpleName
...
//        setToolBar()
    ...
    fun init() {
        ...
        if (isFromEdit) {
            tourbus_Service_Request = CachingManager.getVendorTaxiInfo()
            if (fragmentCount == 1) {
        ...
        when {
            ...
            fragmentManager!!.findFragmentById(R.id.flContentnew) is VendorTaxiTwoFragment -> {
                if (isFromEdit) {
                    ...
                    setTitleBar(resources.getString(R.string.setup_your_bussiness))
    ...
    fun getTourBusFieldPojo(): Ven_Taxi_Service_Request {
        return tourbus_Service_Request
    }
    ...
    fun hitApi(tourbus_Service_Request: Ven_Taxi_Service_Request) {
        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
        tourbus_Service_Request.business_category_id = AppConstants.CAT_TRANSPORTATION.toString()
        tourbus_Service_Request.business_subcategory_id = AppConstants.SUBCAT_TRANS_TOURBUS.toString()
        apiInterface?.taxiServiceSetUp("Bearer " + SharedPreferenceManager.getAuthToken(), tourbus_Service_Request)
    ...
    fun hitApiEdit(tourbus_Service_Request: Ven_Taxi_Service_Request) {
        ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
        tourbus_Service_Request.action = AppConstants.ACTION_EDIT
        apiInterface?.taxiServiceSetUp("Bearer " + SharedPreferenceManager.getAuthToken(), tourbus_Service_Request)
                ?.subscribeOn(Schedulers.io())
                ?.observeOn(AndroidSchedulers.mainThread())
                ?.subscribe(object : Observer<TaxiServiceResponse> {
    ...
    private fun handleResults(userResponse: TaxiServiceResponse) {
        ...
        if (userResponse.status.equals(AppConstants.KEY_SUCCESS)) {
            CachingManager.setVendorTaxiInfo(tourbus_Service_Request)
            if (isFromEdit) {
    ...
    public fun uploadPic(files: ArrayList<File>, bussinessId: String) {
        ...
        if (files != null && files.size > 0) {
            files.subList(0, tourbus_Service_Request.business_images.size).clear()
        }
