...
import com.virtual.customervendor.customer.ui.dialogFragment.SpecialityDialogFragment
...
import com.virtual.customervendor.vendor.ui.fragments.VendorAppOneFragment
import com.virtual.customervendor.vendor.ui.fragments.VendorAppointDoctorFinalFragment
import com.virtual.customervendor.vendor.ui.fragments.VendorAppointTwoFragment
...
class VendorDoctorActivity : BaseActivity(), View.OnClickListener, CityDialogFragment.citySelectionInterface, RegionDialogFragmentSingle.SingleRegionSelectionInterface, UploadBussinessImage.IBussinessImage1, SpecialityDialogFragment.SpecialitySelectionInterface {
    var datetime: String? = null
    var manager: FragmentManager? = null
    var TAG: String = VendorDoctorActivity::class.java.simpleName
    ...
    var specialityDialogFragment: SpecialityDialogFragment? = null
...
//        setToolBar()
    ...
    fun init() {
        ...
        if (isFromEdit) {
            venAppointDoctoreServiceRequest = CachingManager.getVendorDoctorInfo()
            if (fragmentCount == 1) {
    ...
    fun setDisplayDialog(number: Int, title: String, region_id: String) {
        when (number) {
            ...
            9 -> showSpecialitySelectionDialog(title)
        }
    ...
    private fun showSpecialitySelectionDialog(from: String) {
        specialityDialogFragment = SpecialityDialogFragment.newInstance(from)
        manager = supportFragmentManager
        specialityDialogFragment!!.show(manager, "My Dialog")
    }
    ...
    fun getDoctorFieldPojo(): VendorHealthServiceRequest {
        return venAppointDoctoreServiceRequest
    }
    ...
    fun hitApi(doctoreServiceRequest: VendorHealthServiceRequest) {
        if (AppUtils.isInternetConnected(this)) {
            ProgressDialogLoader.progressDialogCreation(this, getString(R.string.please_wait))
            doctoreServiceRequest.business_subcategory_id = AppConstants.SUBCAT_HEALTH_DOCTOR.toString()
    ...
    private fun handleResults(userResponse: TaxiServiceResponse) {
        ProgressDialogLoader.progressDialogDismiss()
        if (userResponse.status.equals(AppConstants.KEY_SUCCESS)) {
            venAppointDoctoreServiceRequest.business_id = userResponse.data.business_id.toInt()
            venAppointDoctoreServiceRequest.service_id = userResponse.data.service_id
            CachingManager.setVendorDoctorInfo(venAppointDoctoreServiceRequest)
            if (imageFiles != null) {
    ...
    override fun specialitySelectionInterface(bean: AppointmentReasonModel, fromWhere: String?) {
        if (specialityDialogFragment != null) {
